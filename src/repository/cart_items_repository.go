...
type CartItemsRepository interface {
	...
	FindByID(cartID int) (entity.CartItems, error)
...
func (r *cartItemsRepository) FindByID(cartID int) (entity.CartItems, error) {
	var getCart entity.CartItems

	err := r.db.Where("cart_id = ?", cartID).First(&getCart).Error
